...
def _check_restore_parameters(sess, saver):
    ...
    if ckpt and ckpt.model_checkpoint_path:
        print("Loading parameters for NMT")
        saver.restore(sess, ckpt.model_checkpoint_path)
    else:
        print("Initializing fresh parameters for NMT")
...
def _load_test_data():
    eng_test_ids = data.load_test_data('tst2013.en')
    vit_test_ids = data.load_test_data('tst2013.vi')
...
def test():
    ...
    with tf.Session() as sess:
        ...
        for i in range(10):
            token_ids = data.sentence2id(enc_vocab, str(eng_test[i]))
        ...
        print("Avg BLEU: {0}".format(np.mean(bleu)))
        print("Avg BLEU %: {0}".format(np.mean(bleu) * 100))
